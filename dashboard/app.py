...
if page == "📡 Overview":
    ...
    with c1:
        ...
        lay.update(yaxis2=dict(overlaying="y", side="right", showgrid=False,
                   ...
                   xaxis=dict(**lay["xaxis"], tickangle=45, nticks=18,
    ...
    with c2:
        ...
        fig2 = go.Figure(go.Pie(
            labels=["Loss of Supply", "Controlled", "Physical"],
            values=[df[df.outage_class==1].shape[0],
                    df[df.outage_class==2].shape[0],
                    df[df.outage_class==3].shape[0]],
            hole=0.4, marker=dict(colors=[C["accent"], C["amber"], C["red"]]),
    ...
    with c3:
        ...
        lay3.update(xaxis=dict(**lay3["xaxis"], showticklabels=False),
                    yaxis=dict(**lay3["yaxis"], tickfont=dict(size=9)),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    lay_l.update(yaxis=dict(**lay_l["yaxis"], title="Probability",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with c2:
        ...
        lay_ct.update(yaxis=dict(**lay_ct["yaxis"], tickfont=dict(size=8)),
                      showlegend=False)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        lay_w.update(xaxis=dict(**lay_w["xaxis"], title="Avg Precipitation (mm)"),
                     yaxis=dict(**lay_w["yaxis"], title="Outage Rate (%)"),
                     showlegend=False)
    ...
    with c2:
        ...
        lay_w2.update(xaxis=dict(**lay_w2["xaxis"], title="Avg Max Wind Gust (m/s)"),
                      yaxis=dict(**lay_w2["yaxis"], title="Outage Rate (%)"),
                      showlegend=False)
    ...
    lay_s.update(yaxis=dict(**lay_s["yaxis"],title="Outage Rate (%)"),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        lay_sf.update(xaxis=dict(**lay_sf["xaxis"],title="Interruptions/customer/year"),
                      yaxis=dict(**lay_sf["yaxis"],tickfont=dict(size=10)),
                      showlegend=False)
    ...
    with c2:
        ...
        lay_sd.update(xaxis=dict(**lay_sd["xaxis"],title="Hours/customer/year"),
                      yaxis=dict(**lay_sd["yaxis"],tickfont=dict(size=10)),
                      showlegend=False)
    ...
    lay_fin.update(yaxis=dict(**lay_fin["yaxis"],title="KES Million"),showlegend=False)
